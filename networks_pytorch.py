...
class Network:
    ...
    def __init__(self, input_dim=0, output_dim=0, lr=0.001, 
        ...
        self.mocel = self.model.to(device)
    ...
    def predict(self, sample):
        ...
        with self.lock:
            self.model.eval()
            with torch.no_grad():
                pred = self.model(torch.from_numpy(sample).float().to(device)).to('cpu').detach().numpy()
                pred = pred.flatten()
            return pred
    ...
    def train_on_batch(self, x, y):
        ...
        loss = 0.
        with self.lock:
            y_pred = self.model(torch.from_numpy(x).float().to(device))
            _loss = self.criterion(y_pred, torch.from_numpy(y).float().to(device))
